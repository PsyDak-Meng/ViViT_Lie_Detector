...
def train(model, epoch, X_train, X_test, y_train, y_test, optimizer, criterion):
    # training
    BATCH_SIZE = 4
    BATCH = ceil(X_train.shape[0] / BATCH_SIZE)
    train_loss = 0
    
    # record least loss epoch
    ...
    for epoch in tqdm(range(20)):
        ...
        while batch_count < BATCH:
            ...
            X_train_batch = X_train_batch.type(torch.float32).to(device)
            y_train_batch = y_train_batch.type(torch.long).squeeze().to(device)
    ...
    model_test.eval() # PyTorch eval I think
